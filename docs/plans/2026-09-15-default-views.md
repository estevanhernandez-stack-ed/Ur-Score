...
- **D20. Version 0.3.1 is this work** (the design). The backlog remediation plan also says 0.3.1; its wave 1 becomes 0.3.2, and its Task 0 re-check covers this branch (S1-14.11 done here, `StarterBoards`, `AccountCard` and `ProfileStat` changed). *Cost if wrong:* a version number in that plan.
