...
**Interfaces:**
- Produces: `Labs626.UrScore.Core.Settings` — `record Settings(string ClanName, string MetricId, int PollSeconds)`, `Settings.Load()`, `Settings.Save()`, `Settings.EffectivePollSeconds` (floored at 180), `Settings.Path`.
...
/// </summary>
public sealed record Settings(string ClanName, string MetricId, int PollSeconds)
{
...
public class ReportPolicyTests
{
    ...
    public void ReportMetricIsCalledFromTheReportPolicyAndNowhereElse()
    {
        ...
        var offenders = Directory.EnumerateFiles(src, "*.cs", SearchOption.AllDirectories)
            .Where(f => !f.EndsWith("ReportPolicy.cs", StringComparison.Ordinal))
            .Where(f => !f.EndsWith("HostClient.cs", StringComparison.Ordinal))   // the implementation itself
            .Where(f => !f.EndsWith("IHostClient.cs", StringComparison.Ordinal))  // the declaration
...
public class ScoreWatchTests
{
    ...
    public async Task DoesNotReplayABacklogWhenTheHostComesBack()
    {
        ...
        // a Level rule would judge on. One cycle, one report, from the value read this cycle.
        var contributions = new ContributionsResult([new(111, 4200)], null);
        var source = new FakeSource(new BattleProbe("B", null), contributions);
        var down = new FakeHost(reachable: false, [new(Mine, 111, "mine")]);
        var watch = Watch(source, down);

        await watch.RunOnceAsync(CancellationToken.None);
        await watch.RunOnceAsync(CancellationToken.None);
        await watch.RunOnceAsync(CancellationToken.None);

        var up = new FakeHost(reachable: true, [new(Mine, 111, "mine")]);
        await Watch(source, up).RunOnceAsync(CancellationToken.None);

        Assert.Single(up.Reported);
    }
...
public partial class MainWindow : Window
{
    ...
    private void Render(WatchSnapshot snapshot)
    {
        ...
        foreach (var line in snapshot.Accounts)
        {
            ...
            if (row is null)
            {
                row = new Row { AccountId = line.AccountId, DisplayName = line.DisplayName };
                _rows.Add(row);
            }
