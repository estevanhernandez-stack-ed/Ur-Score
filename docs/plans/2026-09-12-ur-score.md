...
> **DONE, and the code below is no longer the truth — `src/Source/` is.** Shipped at `6a43407`
> after two fix rounds. Four things changed and each is worth knowing before reading the blocks
> below, because all four were defects in this plan text rather than in the implementation:
>
> - **`Unwrap` accepts a null wrapper**, and prefers an object one when both are present.
>   `{"status":"ok","data":null}` is how this API says "no battle running", and the object-only
>   version read that as a shape failure.
> - **A non-empty array where every row fails now returns a Miss**, not a clean empty list. The
>   version below returns `(empty, null)` regardless, which is indistinguishable from a battle
>   nobody has scored in — the one outcome this parser exists to prevent.
> - **`TryUserId` replaces the `(long)` cast.** Casting a double fabricated `long.MaxValue` from
>   `1e20` and kept the row, reporting a corrupt id against the wrong account.
> - **A failed row records WHY, not just which keys it had.** Naming the keys is right when a key
>   is absent and misleading when the keys are present and a value was wrong.
>
> Reasoning and the live reproductions are in `.superpowers/sdd/2026-09-12-ur-score/progress.md`.
...
internal static class JsonNav
{
    ...
    public static JsonElement Unwrap(JsonElement root)
    {
        ...
        {
            // Null counts as a wrapper value, not as an absent one. `{"status":"ok","data":null}`
            // is how this API says "no battle running", and an object-only test let that fall
            // through to the un-wrapped root — where the absent configName then read as a shape
            // failure instead of the normal, frequent, not-an-error state it is.
...
- Uses Task 2's trailing `bool MissIsTransport = false`, which Task 2 declares on both result records and never sets. This task is the only thing that sets it, on its own transport failures; a parser miss leaves it false. Task 8's loop needs that distinction because "could not reach it" and "could not read it" have nothing in common as remedies.
...
> **REWRITTEN 2026-09-12 for spec §6.1.** This was a diagnostic panel for the alert pipeline, which
> is useless to anyone not configuring an alert. It is now a clan battle dashboard first — clan Place
> and total, every contributor ranked with the user's own accounts marked, names resolved, and a
> per-member rate — with the diagnostic half beneath it. It therefore consumes **Task 11**
> (`ClanStanding`) and **Task 12** (`NameClient`), so those two run BEFORE this one.
>
> **Execution order for the remaining tasks: 5, 6, 7, 8, 11, 12, 9, 10.**
>
> The XAML and code-behind below still describe the diagnostic-only window. Treat them as the
> starting point for the lower half and write the dashboard above it, taking the layout from spec
> §6.1's table. Two things from that section are requirements rather than suggestions:
>
> - **The rate shown is not the rate RoRoRo judges**, and the window must say so where it is shown.
>   The plugin's figure comes from its own last two samples; RoRoRo derives its own over the window
>   the user's rule names. They will disagree sometimes and that is not a bug.
> - **The source is credited.** A window on one machine is not public display and does not trigger
>   the vendor's attribution clause, but one line of credit costs nothing and the alternative is a
>   tool presenting someone else's work as its own.
...
### Task 11: The clan's standing, and the ranking

Added 2026-09-12 with spec §6.1. Everything here comes out of a response the parser already reads
and steps past, so this task adds no network call and no new failure mode — it stops discarding.

**Files:**
- Create: `src/Source/ClanStanding.cs`
- Create: `tests/ClanStandingTests.cs`

**Interfaces:**
- Consumes: `Contribution` (Task 2), and the same `JsonNav` helpers.
- Produces:
  - `record ClanStanding(long? Place, double? Points)` — both nullable, because a battle can be live
    before either is populated and a missing one is not a failure.
  - `record RankedContribution(int Position, long UserId, double Points, bool IsMine)`
  - `ClanStanding.Read(string json, string configName)`
  - `ClanStanding.Rank(IReadOnlyList<Contribution> contributions, IReadOnlySet<long> mine)`

- [ ] **Step 1: Write the failing standing tests**
...
Run: `dotnet test tests/Ur-Score.Tests.csproj --filter ClanStandingTests`
Expected: FAIL — `ClanStanding` does not exist.

- [ ] **Step 3: Write `src/Source/ClanStanding.cs`**

Both methods are static members **on the record**, not on a separate static class — a record and a
static class cannot share a name, and inventing a second type name for two functions would be worse
...
- [ ] **Step 4: Run the standing tests until green**
...
```bash
git add -A
git commit -m "feat(source): read the clan's place and rank the contributors"
```
...
### Task 12: Names from Roblox, in one call, cached

Added 2026-09-12 with spec §6.1. A leaderboard of user ids is unreadable and nobody recognises
their own clanmates in it.

**This is the plugin's second outbound endpoint, and the first that carries other people's
identifiers.** Spec §6.1 sets out why that is acceptable and states it plainly rather than burying
it: ids Roblox issued go to Roblox to retrieve names Roblox publishes, and they came from a public
endpoint to begin with. It is still switchable off.

**Files:**
- Create: `src/Source/NameClient.cs`
- Create: `tests/NameClientTests.cs`
- Modify: `src/Core/Settings.cs` — add a trailing optional `ResolveNames`

**Interfaces:**
- Produces:
  - `INameSource` with `Task<IReadOnlyDictionary<long, string>> ResolveAsync(IReadOnlyCollection<long> ids, CancellationToken)`
  - `NameClient(HttpClient http)` implementing it, plus `NameClient.BatchLimit` and `NameClient.Endpoint`
- Amends Task 1: `Settings` gains a trailing `bool ResolveNames = true`. Trailing and optional, so
  every `Settings` construction in Tasks 1, 8 and 11 compiles unchanged.

**The response shape, verified live 2026-09-12** — not taken from documentation, because this
project has twice been burned by a documented shape that did not match the wire:
...
`name` is the username; `displayName` is the display name. This task takes `name`, because a
leaderboard is a roster and the username is the stable identity a clanmate is known by.

- [ ] **Step 1: Write the failing name-client tests**
...
Run: `dotnet test tests/Ur-Score.Tests.csproj --filter NameClientTests`
Expected: FAIL — `NameClient` does not exist.

- [ ] **Step 3: Write `src/Source/NameClient.cs`**
...
public sealed class NameClient(HttpClient http) : INameSource
{
    ...
    private sealed record NameRow(long Id, string? Name);
...
- [ ] **Step 4: Add `ResolveNames` to `Settings`**

A trailing optional parameter, so nothing else that constructs `Settings` changes:
...
Add one test asserting `Settings.Defaults.ResolveNames` is true and that it survives a disk round
trip when set false.

- [ ] **Step 5: Run the name tests until green**
...
```bash
git add -A
git commit -m "feat(source): resolve clan member names in one cached call"
```
