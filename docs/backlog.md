...
- **OPEN: 122** (19 you'd notice, 103 code, tests, performance or docs only), including 5 found since v0.3.0 shipped (V3-S.2 to V3-S.17)
- **FIXED: 81**
- **GONE: 7**
- Total: 210 distinct items (duplicates merged; every source is named on the line)
- Counted from the rows on 2026-09-16, after V3-S.6, V3-S.7 and S1-14.1 closed and V3-S.18 was found and fixed, not adjusted: each **OPEN**, **FIXED** and **GONE** row from "Stage 1" to the end of "Stage 2", and the lines of the two summary lists below.
...
- V3-S.17 **OPEN** — you'd notice: the same defect one button along. **Choose another**, which a panel shows when its source or stat is gone, is called "Choose another" on every panel that shows it, so two stale panels offer two buttons a screen reader can't tell apart. It was left out of V3-S.16 to keep that change to the two tools the owner named; the rule and the helper are already there (`BoardText.PopOutName`) — src/UI/Panels/PanelFrame.xaml — noted while fixing V3-S.16, 2026-09-16
