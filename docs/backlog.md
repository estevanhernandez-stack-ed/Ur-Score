...
- V3-S.10 **OPEN — five of the seven stock boxes are gone; two are held back by tools/smoke.** Done 2026-09-16: a question now goes through Ur Score's own themed `ConfirmWindow`, whose words are decided in `Confirm` and tested away from the window — remove a recipe (RecipesPage.xaml.cs) and add a 6th clan (ClansPage.xaml.cs; S1-11.4). Something that has already gone wrong is not a question and no longer opens anything: a failed board save stays on the board's own detail line until a save works (BoardWindow.xaml.cs `SaveBoards`) and a recipe that couldn't be saved is said under Import recipe… (ImportFlow.cs, RecipesPage.xaml `ImportProblemLine`), the way Setup › Alerts says a failed write on the card. `MessageBoxFenceTests` bans `MessageBox.Show` across all of src with exactly two rows in its allow list. Still stock: BoardWindow.xaml.cs:414 (Delete board) and ImportFlow.cs:152 (the refused-import warning) — tools/smoke/walk-board-editing.ps1 step 9 and tools/smoke/window-smoke.ps1 step 2 drive those two through Win32 (`Close-MessageBox` posts BM_CLICK to a control's window handle, `Get-MessageBoxText` reads a `Static`), and a WPF window has neither, so each needs its script changed in the same commit. Still open too: the default-styled ToolTips on PanelFrame.xaml:8 ("Drag to move") and PanelPopOutWindow.xaml:19 ("Return to the board"). All eight Ur Score windows already follow the theme and Ur Score has no toasts; the recipe file picker (ImportFlow.cs:26) is Windows' own dialog and follows the Windows theme, which Ur Score can't restyle — owner's first look, 2026-09-15, five converted 2026-09-16
