...
| `walk-score-book.ps1 [-Main CCGP]` | Setup › Score book counts, the not-recording reason, book files, Export stats to a file and Import stats of the same file back ("nothing new"), the setup import through its preview on a second, fresh folder (the recipe ticked, the clan unticked, sends off, the aside folder), diagnostics without book content, the privacy check |
